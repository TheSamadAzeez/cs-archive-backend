...
export class AuthSupervisorService {
  ...
  async refreshSupervisorAccessToken(refreshToken: string) {
    ...
    const tokenRecord = await this.drizzle.db.query.refreshSupervisorTokens.findFirst({
  ...
  async revokeSupervisorRefreshToken(token: string) {
    ...
    await this.drizzle.db.update(refreshSupervisorTokens).set({ revoked: true }).where(eq(refreshSupervisorTokens.token, token));
